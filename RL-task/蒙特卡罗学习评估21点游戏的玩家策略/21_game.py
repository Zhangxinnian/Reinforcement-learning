...
A = ['继续叫牌','停止叫牌']
display = False
#Create a player and a dealer, the player uses the original strategy, and the dealer uses its fixed strategy
player = Player(A = A, display = display)
dealer = Dealer(A = A, display = display)
#Create a scene
arena = Arena(A = A, display = display)
#Generate num complete games
arena.play_games(dealer, player, num=200000)

#Count the value of each state, the attenuation factor is 1, the instant reward of the intermediate state is 0, incremental Monte Carlo evaluation
def policy_evaluates(episodes, V, Ns):
...
policy_evaluates(arena.episodes, V, Ns) #Learning V value
...
def draw_value(value_dict, useable_ace = True, is_q_dict = False, A = None):
    ...
    plt.show()
draw_value(V, useable_ace = True, A = A) #Draw a state value map when A is available
draw_value(V, useable_ace = False, A = A) #Draw a state value map when no A is available

#Observe several game information
display = True
